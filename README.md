...
```
Authentication is needed to validate the identity. The authorization process determines what data you can access.
```

2. What is the point of a [JSON Web Token](https://jwt.io/introduction)? Why would we want to use it?

```
a web token can make sure that you are sharing information securely and lets user to only acess information that they are allwowed.
```

3. Why would we hash a user's password when they sign up? What's the point?

```
It is more secure due to the users password isnt stored. 
```
