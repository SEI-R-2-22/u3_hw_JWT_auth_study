...
```So that we can have access to account specific data for individual users. Anything from credit card information to social security to job work.  
```

2. What is the point of a [JSON Web Token](https://jwt.io/introduction)? Why would we want to use it?

```
We use them to ensure that a password can't be changed or messed with once the token is altered. 
```

3. Why would we hash a user's password when they sign up? What's the point?

```
So that we never actually have to store a user's private password. 
```
