...
```
Because only a right user can access the account
```

2. What is the point of a [JSON Web Token](https://jwt.io/introduction)? Why would we want to use it?

```
JWT  is an encrypted container format . We would want to use this because it securely transfer data on the internet
```

3. Why would we hash a user's password when they sign up? What's the point?

```
we never store the password in the database. Security risk
```
