...
```
To verify the identity of users, in order to secure their information, and match the correct information to the correct user.
```

2. What is the point of a [JSON Web Token](https://jwt.io/introduction)? Why would we want to use it?

```
For authorization and information exchange purposes.
```

3. Why would we hash a user's password when they sign up? What's the point?

```
To verify the intergrity of users password sent during login, against an already created data fingerprint, so that the actual password is never stored and the integrity of data is protected.
```
