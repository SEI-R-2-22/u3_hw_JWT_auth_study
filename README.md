...
```
Authentification is an important part of of verifying user identity in our web app.
```

2. What is the point of a [JSON Web Token](https://jwt.io/introduction)? Why would we want to use it?

```
A json web token is needed for securely transfering data between parties. We would want to use it when storing user data like passwords email etc.
```

3. Why would we hash a user's password when they sign up? What's the point?

```
We hash a user's password so we can store the data without it being able to be tampered with. The point is so your actual password is never actually stored instead it checks the hash.
```
